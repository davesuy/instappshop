(() => {
  ...
  const NotifyScript = (jQuery) => {
    ...
    let scrore = 0,
      prevRun = false;
    const cookieKey = "prev_order",
      createOrder = (data, plan_id, callback) => {
        jQuery.ajax({
          url: `${cUrl}/tienda/${LS.store.id}`,
          type: "post",
          dataType: "json",
          data: {
            items: data.items,
    ...
    jQuery.ajax({
      ...
      success: function(response) {
        if (response) {
          ...
          let feedContianer = `<div class="insta-feed-attcher">`; //create Div Element w/ jquery
          jQuery.each(feeds, function(index, feed) {
            if (feed.type == "image") {
              feedContianer += `<div><img src="${feed.url}" alt="${feed.caption}"></div>`;
            }
          });

          feedContianer += `</div>`;
          instafeedContainer = jQuery("body").find(".insta-feed-container")
            .length
